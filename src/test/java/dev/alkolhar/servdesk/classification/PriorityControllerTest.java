...
class PriorityControllerTest {
	...
	void requestsWithoutCredentialsAreRejected() {
		ResponseEntity<String> response = restTemplate.getForEntity("/api/priorities", String.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);
	}

	@Test
	void createsAndReadsAPriority() {
		Map<String, Object> request = Map.of("name", "Critical", "sortOrder", 0);
		ResponseEntity<Map> created = asAdmin().postForEntity("/api/priorities", request, Map.class);
		...
		ResponseEntity<Map> fetched = asAdmin().getForEntity("/api/priorities/" + id, Map.class);
	...
	void rejectsInvalidPayload() {
		...
		ResponseEntity<String> response = asAdmin().postForEntity("/api/priorities", request, String.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
	}

	@Test
	void returnsNotFoundForMissingPriority() {
		ResponseEntity<String> response = asAdmin().getForEntity("/api/priorities/999999", String.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
	}

	@Test
	void updatesAndDeletesAPriority() {
		Number id = (Number) asAdmin()
				.postForEntity("/api/priorities", Map.of("name", "Temp", "sortOrder", 5), Map.class).getBody()
				.get("id");

		Map<String, Object> updateRequest = new HashMap<>();
		updateRequest.put("name", "Updated");
		updateRequest.put("sortOrder", 2);
		ResponseEntity<Map> updated = asAdmin().exchange("/api/priorities/" + id, HttpMethod.PUT,
		...
		asAdmin().delete("/api/priorities/" + id);
		ResponseEntity<String> afterDelete = asAdmin().getForEntity("/api/priorities/" + id, String.class);
	...
	void customersCanReadButNotCreateUpdateOrDelete() {
		Number id = (Number) asAdmin()
				.postForEntity("/api/priorities", Map.of("name", "Low", "sortOrder", 9), Map.class).getBody().get("id");

		ResponseEntity<String> createResponse = asCustomer().postForEntity("/api/priorities",
				Map.of("name", "Not allowed", "sortOrder", 1), String.class);
		assertThat(createResponse.getStatusCode()).isEqualTo(HttpStatus.FORBIDDEN);

		ResponseEntity<Map> readResponse = asCustomer().getForEntity("/api/priorities/" + id, Map.class);
		assertThat(readResponse.getStatusCode()).isEqualTo(HttpStatus.OK);

		Map<String, Object> updateBody = new HashMap<>();
		updateBody.put("name", "Not allowed");
		updateBody.put("sortOrder", 1);
		ResponseEntity<String> updateResponse = asCustomer().exchange("/api/priorities/" + id, HttpMethod.PUT,
				new HttpEntity<>(updateBody), String.class);
		assertThat(updateResponse.getStatusCode()).isEqualTo(HttpStatus.FORBIDDEN);

		ResponseEntity<String> deleteResponse = asCustomer().exchange("/api/priorities/" + id, HttpMethod.DELETE, null,
				String.class);

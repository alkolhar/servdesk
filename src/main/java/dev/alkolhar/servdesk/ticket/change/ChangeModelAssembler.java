...
@Component
public class ChangeModelAssembler implements RepresentationModelAssembler<Change, ChangeModel> {

	@Override
	public ChangeModel toModel(Change change) {
		Ticket ticket = change.getTicket();
		ChangeModel model = new ChangeModel();
		model.setId(change.getId());
		model.setDisplayNumber(change.getDisplayNumber());
